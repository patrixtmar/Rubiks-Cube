...
String result = "";  // The solution from Thistlethwaite two phase algorithm
String fresult = ""; // The solution from Thistlethwaite two phase algorithm
...
Thistlethwaite() 
  { 
	...
	//cubeString = "555555666555666666334334334344344344222222222111111111"; 
	//Cube cube = Utils.deserializeCube("555555666555656666334334334344344344222222222111111111");
     if (DebugSetting == 1)
     {
        System.out.println(cubeString); 
     }
     System.out.println("----------------------- Thistlethwaite Test Start ------------------------------");
     ...
     System.out.println(printcubeStringL.substring(0, 3) + " " + printcubeStringB.substring(0, 3) + " " + printcubeStringR.substring(0, 3)+ " " + printcubeStringF.substring(0, 3));
     System.out.println(printcubeStringL.substring(3, 6) + " " + printcubeStringB.substring(3, 6) + " " + printcubeStringR.substring(3, 6)+ " " + printcubeStringF.substring(3, 6));
     System.out.println(printcubeStringL.substring(6, 9) + " " + printcubeStringB.substring(6, 9) + " " + printcubeStringR.substring(6, 9)+ " " + printcubeStringF.substring(6, 9));
	...
	//cubeString="166166144233255255666333333544544544311511511224226226";
     //cubeString="343461112215555412154331625642546466663513431525226233";
     //  Motor.B.rotateTo(20); //Move color sensor arm out of way for cube movement
     startcalculate = System.currentTimeMillis();
     Calculate(); // Calculate solution use Thistlethwaite algorithm
     ...
     System.out.println("----------------------- Thistlethwaite Test Finish ------------------------------");
...
////Method to assign a color to a side \\\\ 
 ...
 public String ScanColor() // Returns the side which the facelet be-longs to in a string.
  { 
        ...
        {
            if (BlueValue <= 60) side = "6"; 
            else side = "5"; 
     	
        }
        if (ColorIDValue == 0 || ColorIDValue == 5)  //Red or Orange
        {
            if (GreenValue <= 150) side = "4"; 
            else side = "3"; 
...
void CubeStringU() // Used for White or Up side
  { 
        ...
        cubeStringU += f3; 
        cubeStringU += f4; 
        cubeStringU += f5; // For the facelet with the Rubik's Cube Logo.
        cubeStringU += f6; 
        cubeStringU += f7; 
        cubeStringU += f8; 
        cubeStringU += f9;
...
public static String TranslateCube(String StringMove){
	...
	case "L1": RetValue = "L"; break;
	case "L2": RetValue = "L2"; break;
	case "L3": RetValue = "L'"; break;
	case "R1": RetValue = "R"; break;
	case "R2": RetValue = "R2"; break;
	case "R3": RetValue = "R'"; break;
...
public static String PrintTranslateCube(String StringMove){
	String RetValue = "";
	switch (StringMove){
	case "U1": RetValue = "W"; break;
	case "U2": RetValue = "W2"; break;
	case "U3": RetValue = "W3"; break;
	case "D1": RetValue = "Y"; break;
	case "D2": RetValue = "Y2"; break;
	case "D3": RetValue = "Y3"; break;
	case "L1": RetValue = "O"; break;
	case "L2": RetValue = "O2"; break;
	case "L3": RetValue = "O3"; break;
	case "R1": RetValue = "R"; break;
	case "R2": RetValue = "R2"; break;
	case "R3": RetValue = "R3"; break;
	case "F1": RetValue = "G"; break;
	case "F2": RetValue = "G2"; break;
	case "F3": RetValue = "G3"; break;
	case "B1": RetValue = "B"; break;
	case "B2": RetValue = "B2"; break;
	case "B3": RetValue = "B3"; break;
	}
   return RetValue;
}    
...
public static String PrintCube(String StringColor){
	String RetValue = "";
	switch (StringColor){
	case "1": RetValue = "B"; break;
	case "2": RetValue = "G"; break;
	case "3": RetValue = "O"; break;
	case "4": RetValue = "R"; break;
	case "5": RetValue = "W"; break;
	case "6": RetValue = "Y"; break;
	}
   return RetValue;
}    
...
void Calculate()  // Passes all facelets to Thistle
  { 
    if (DebugSetting == 1)
    {
        System.out.println("cubeString="+cubeString); 
	    System.out.println(TranslatedSolution+" Before"); 
    }

    	//Cube cube = TestUtils.generateRandomValidCube();
		Cube cube = Utils.deserializeCube(cubeString);
		...
		for (int i = 0; i < Thistle.solution.size(); i++) {
			StoreTranslatedSolution(TranslateCube(Thistle.solution.get(i)));
			PrintStoreTranslatedSolution(PrintTranslateCube(Thistle.solution.get(i)));
		}

        result = TranslatedSolution;
        fresult = result;
        if (DebugSetting == 1)
        {
    	    System.out.println(TranslatedSolution+" After"); 
        }   
